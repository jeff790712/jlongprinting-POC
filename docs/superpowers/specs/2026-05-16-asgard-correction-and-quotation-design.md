...
| **Heimdall** | Asgard Media & PR AI | 媒體與輿情 AI |
...
[A] Asgard 平台訂閱
  欄位：方案 | 月費 | 月數 | 小計
  Basic-Lite | NT$20,000 | ___ | =B×C
  附註：依正式上線使用量可升級至 Regular / Plus

[B] 一次性導入服務（4-6 週，分三階段）
...
[C] Phase 2 進階模組（另行報價，不計入本次總價）
  Sindri Agent Hub（多 Agent 編排）
  Mimir Data Insight（數據洞察儀表板）
  Heimdall Media & PR AI（輿情監測）
  → 此區塊金額欄位填「另行報價」字串
...
- **zh-TW 維持**：所有修改保持繁體中文
- **品牌色維持**：Odin 藍 #1e3a8a 在 Phase 1 強調；Sindri 紫 #5b21b6、Mimir 綠 #065F46 在 Phase 2 區塊使用；Heimdall 暫採用霧灰 #475569（如後續官網有正式色彩再替換）
...
- [ ] 將「訂閱月數 = 2」+「Phase A/B/C 各填 1 人天」帶入後，加總區公式正確：訂閱小計 NT$ 40,000、一次性導入小計 NT$ 60,000、未稅 NT$ 100,000、營業稅 NT$ 5,000、含稅 NT$ 105,000
